package e2;

public class NodoTormentaMarina extends NodoRutaFija{

    public NodoTormentaMarina(char nombre, Nodo hijo, int ataque) {
        super(nombre, hijo, ataque);
    }

    @Override
    public void simulacion(Barco barco) {

        if (barco == null){
            System.out.println("Barco nulo.");
        }else{

            if (barco.getVision() < this.getAtaque()){
                barco.setHp(barco.getHp() - 10);
            }

            if (barco.getHp() <= 0){
                printResult (false, barco, this.getNombre());
            }else{

                if (this.getHijo() == null){
                    System.out.println("Nodo siguiente nulo, hijo de " + this.getNombre() + ". Fin de incursion");
                }else{
                    this.getHijo().simulacion(barco);
                }
            }
        }

    }

    @Override
    public void mostrarMapa() {
        System.out.print("(" + this.getNombre() + " Maelstrom, ");

        this.getHijo().mostrarMapa();

        System.out.print(")");
    }
}
